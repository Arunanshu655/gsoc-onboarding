...
val displayOrder: List[Ingr] = List (
   Ingr.Bun,  Ingr.Pickle, Ingr.Tomato, Ingr.Cheese, Ingr.Patty, Ingr.Lettuce,
)
...
val loladenney: Contributor = 
    Contributor("loladenney"):
        Resource.eval(initBurger).flatMap { ingredients =>
            div(
                p("Hello, I'm @loladenney on GitHub and L otherwise (please ignore the github deadname haha).", br(()), "I agree to follow the Typelevel CoC and GSoC AI policy."),
                    ...
                    displayOrder.map{ ingr =>
                        button(
                            ...
                            styleAttr <-- ingredients(ingr).active.map { active =>
                                if active then "background-color: rgb(204, 255, 153);" else "background-color: rgb(255, 102, 102);"
                            },
                ...
                div(
                    // burger display
                    styleAttr := "margin:0",
                    pre(displayOrder.map{ ingr =>
                        val rep = ingredients(ingr)
                        rep.active.map { isactive => 
                            if isactive then rep.ascii else ""
                        }
                    },
